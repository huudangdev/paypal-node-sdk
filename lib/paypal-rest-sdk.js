...
module.exports = function() {
    ...
    function generateToken(client_id, client_secret, cb) {
        ...
        var token = null;
        invoke('https', 'POST', '/v1/oauth2/token', 'response_type=token&grant_type=client_credentials', http_options, function(res, err) {
            if (err) {
                throw err;
            }
    ...
    function executeHttp(http_scheme, http_method, path, data, http_options, cb) {
        ...
        if (http_options.headers.Authorization) {
            invoke(http_scheme, http_method, path, data, http_options, function(response, error) {
