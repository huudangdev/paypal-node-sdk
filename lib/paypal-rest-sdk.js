...
module.exports = function () {
    ...
    function generateToken(client_id, client_secret, config_opts, cb) {

        var basicAuthString = 'Basic ' + new Buffer(client_id + ':' + client_secret).toString('base64');

        var http_options = {
            host: (config_opts && config_opts.host != null) ? config_opts.host : default_options.host,
            port: (config_opts && config_opts.port != null) ? config_opts.port : default_options.port,
            headers: {
    ...
    function update_token(http_options, callback) {
        generateToken(http_options.client_id, http_options.client_secret, http_options, function (token) {
    ...
    return {
        ...
        generateToken: function (client_id, client_secret, config_opts, cb) {
            generateToken(client_id, client_secret, config_opts, cb);
        },
        payment: {
